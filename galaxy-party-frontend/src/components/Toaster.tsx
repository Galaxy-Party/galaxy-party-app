...
const STYLES: Record<ToastType, { border: string; icon: string; glow: string }> = {
  error:   { border: 'border-l-rose border-rose/20',          icon: 'text-rose',       glow: 'shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_0_1px_rgba(244,114,182,0.08)]' },
  success: { border: 'border-l-emerald border-emerald/20',    icon: 'text-emerald',    glow: 'shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_0_1px_rgba(52,211,153,0.08)]'  },
  warning: { border: 'border-l-amber border-amber/20',        icon: 'text-amber',      glow: 'shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_0_1px_rgba(251,191,36,0.08)]'  },
  info:    { border: 'border-l-indigo border-indigo/20',      icon: 'text-indigo',     glow: 'shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_0_1px_rgba(129,140,248,0.08)]' },
}
    ...
    <div className="fixed top-5 right-5 z-[9999] flex flex-col gap-[10px] pointer-events-none">
      {toasts.map(toast => {
        ...
        return (
          ...
          >
            {ICONS[toast.type]}
            <span className="flex-1 text-sm leading-[1.5] text-text break-words">
              {toast.message}
            </span>
            <button
              onClick={() => dismiss(toast.id)}
              className="shrink-0 flex items-center justify-center w-5 h-5 rounded-md text-text-dim hover:text-text/80 transition-colors duration-150 mt-[1px]"
            >
