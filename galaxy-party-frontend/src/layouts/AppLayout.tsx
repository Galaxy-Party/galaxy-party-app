...
import { useLayoutSession } from './hooks/useLayoutSession'
...
export default function AppLayout() {
  ...
  const { hasNotif, clearNotif, gameInvite, respondAccept, respondDecline } = useLayoutSession(friendsOpen)
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-bg">
      ...
      {gameInvite && (
        <GameInviteNotif
          invite={gameInvite}
          onAccept={() => respondAccept(gameInvite.inviteId)}
          onDecline={() => respondDecline(gameInvite.inviteId)}
        />
      )}
      <Starfield />
      <Nebulae teal />
      ...
      <div className="relative z-[1] w-full h-full flex flex-col">
        ...
        <header className="w-full flex items-center justify-between px-8 pt-5 shrink-0">
          ...
          {user && (
            <div className="flex items-center gap-[10px]">
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'linear-gradient(135deg,var(--color-indigo-deep),var(--color-purple))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Space Grotesk', sans-serif", fontSize: 10, fontWeight: 700, color: 'white', flexShrink: 0, boxShadow: '0 0 8px rgba(129,140,248,0.4)' }}>
                      {level}
                    </div>
                    <div style={{ width: 72, height: 4, borderRadius: 2, background: 'rgba(129,140,248,0.15)', overflow: 'hidden', flexShrink: 0 }}>
                      <div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg,var(--color-indigo),var(--color-rose))', width: `${progress}%`, transition: 'width 0.6s ease' }} />
                    </div>
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 10, color: 'var(--color-text-dim)', whiteSpace: 'nowrap' }}>
              ...
              {/* Friends button */}
              <button
                onClick={() => { setFriendsOpen(o => !o); clearNotif() }}
                className={`relative flex items-center gap-2 backdrop-blur-[14px] rounded-[30px] py-[7px] pr-4 pl-3 transition-all duration-200 ${friendsOpen ? 'bg-indigo/15 border border-indigo' : 'bg-panel/92 border border-indigo/45'}`}
              >
                <svg viewBox="0 0 24 24" fill="none" stroke={friendsOpen ? 'var(--color-indigo)' : 'rgba(241,240,255,0.85)'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-[18px] h-[18px] shrink-0">
                  <circle cx="9" cy="7" r="4"/><path d="M3 21v-2a4 4 0 014-4h4a4 4 0 014 4v2"/>
                  <path d="M16 3.13a4 4 0 010 7.75"/><path d="M21 21v-2a4 4 0 00-3-3.85"/>
                </svg>
                <span className={`font-display text-[13px] font-semibold ${friendsOpen ? 'text-indigo' : 'text-text'}`}>Amis</span>
                {hasNotif && <span className="absolute top-[6px] right-[8px] w-2 h-2 rounded-full bg-rose shadow-[0_0_6px_var(--color-rose)]" />}
              </button>

              {/* Player tag */}
              <button
                onClick={() => navigate('/profile')}
                className="flex items-center gap-[9px] bg-panel/92 backdrop-blur-[14px] border border-indigo/45 rounded-[30px] py-[7px] pr-4 pl-[10px] cursor-pointer transition-all duration-200 hover:border-indigo hover:bg-indigo/10"
              >
                <div className="w-7 h-7 rounded-full bg-navy border border-indigo overflow-hidden flex items-center justify-center shrink-0">
                  <img src={user.imageName ?? undefined} alt="avatar" className="w-[80%] h-[80%] object-contain" />
                </div>
                <span className="font-display text-[13px] font-semibold text-text">{user.username}</span>
                <div className="w-[7px] h-[7px] rounded-full bg-emerald shadow-[0_0_6px_var(--color-emerald)] shrink-0" />
              </button>
            </div>
          )}
        </header>
        ...
        <div className="shrink-0 flex justify-center px-8 pb-6 pt-[10px] w-full">
          <div className="flex gap-[6px] bg-panel/95 backdrop-blur-[24px] border border-indigo/40 rounded-[20px] p-[7px] shadow-[0_8px_40px_rgba(0,0,0,0.7)] w-[min(940px,92vw)]">

            {dockItems.map(item => {
              const active = pathname === item.path
              const isRanked = 'ranked' in item && item.ranked
              const activeColor = isRanked ? 'var(--color-amber)' : 'var(--color-indigo)'
            ...
            >
              <div className="opacity-70 text-text/75">
              ...
              <span className="text-[12px] font-semibold uppercase tracking-[0.05em] text-center text-text/75">Règles</span>
              <span className="text-[10px] text-center text-text/40">Comment jouer ?</span>
            </button>

            <div className="w-px bg-indigo/30 my-1 self-stretch shrink-0" />

            <button
              onClick={logout}
              className="flex flex-col items-center gap-[2px] py-[10px] px-4 flex-1 rounded-[14px] border border-transparent bg-transparent transition-all duration-200"
            >
              <div className="opacity-80 text-rose">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-5 h-5">
              ...
              <span className="text-[12px] font-semibold uppercase tracking-[0.05em] text-center text-rose opacity-90">Déconnexion</span>
              <span className="text-[10px] text-center text-rose/55">Quitter</span>
            </button>
